...
class OrderServiceTest extends AnyWordSpec with Matchers with OrderService
  ...
  "The order service" should {
    ...
    "return the tracking order for an order that was posted" in {
      ...
      } // http POST http://localhost:5000/orders Content-Type:text/xml <<< '<order><customerId>Customer 1</customerId><productId>Akka in Action</productId><number>20</number></order>'
