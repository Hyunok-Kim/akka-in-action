...
object JobReceptionist {
  ...
  case class JobRequest(name: String, text: List[String]) extends MySerializable

  sealed trait Response extends MySerializable
  case class JobSuccess(name: String, map: Map[String, Int]) extends Response
  case class JobFailure(name: String) extends Response

  case class WordCount(name: String, map: Map[String, Int]) extends MySerializable
