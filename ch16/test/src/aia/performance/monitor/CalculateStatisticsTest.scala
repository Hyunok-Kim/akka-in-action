...
class CalculateStatisticsTest extends AnyWordSpecLike
  ...
  "getMailboxSummary" must {
    ...
    "calculate statistics when one event" in {
      ...
      val stat = CalculateStatistics.processMailboxEvents(duration,
        mailboxEvents)
  ...
  "getActorSummary" must {
    ...
    "calculate statistics with different service times" in {
      val startTime = System.currentTimeMillis()
      val duration = 1.second
    ...
    "calculate statistics with boundary events" in {
      val startTime = System.currentTimeMillis()
      val duration = 1.second
      val serviceTime = 333L
      val actorEvents = Seq(
