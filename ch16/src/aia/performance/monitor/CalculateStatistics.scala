...
object CalculateStatistics {
  ...
  case class ActorSummary(utilization: Double, averageServiceTime: Double)

  def processActorEvents(startTimePeriod: Long, period: Duration,
    ...
    if (nrEvents == 0 || period.toMillis == 0) {
      ...
      ActorSummary(utilization = utilization.toDouble,
        averageServiceTime =
          if (nrProcess != 0)
            (totalProcessTime / nrProcess).toDouble
          else 0.0)
    }
  }
  ...
  def processMailboxEvents(period: Duration,
    ...
    if (nrEvents == 0 || period.toSeconds == 0) {
      ...
      MailboxSummary(maxQueueLength = maxQueue,
        arrivalRate = (nrEvents / period.toSeconds).toDouble,
        averageWaitTime = (totalWaitTime / nrEvents).toDouble)
    }
  }
}
