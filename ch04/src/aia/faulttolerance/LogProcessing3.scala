...
object LogProcessingApp extends App {
  ...
  val topLevelProps = Props(new FileWatcherSupervisor(sources,
    logProcSuperProps))
  system.actorOf(topLevelProps)
}

class FileWatcherSupervisor(sources: Vector[String], logProcSuperProps: Props)
  extends Actor {
...
class DbCon(url: String) {
   ...
   * @throws DbNodeDownException 데이터베이스 노드가 데이터베이스 클러스터에서 제거된 경우 발생한다. 결코 노드가 다시 정상 작동하게 되는 일은 없다.
   */
