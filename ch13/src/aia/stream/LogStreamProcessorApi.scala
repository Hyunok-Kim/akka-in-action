...
class LogStreamProcessorApi(
  ...
  // in a realistic application this would be a Sink to some other
  ...
  def processEvents(logId: String): FlowLike = {
    ...
    Flow.fromGraph(
      GraphDSL.create() { implicit builder =>
        ...
        def rollupErr = rollup(nrErrors, errDuration)
        def rollupWarn = rollup(nrWarnings, warnDuration)
        ...
        post {
          entity(as[Source[Event, _]]) { src =>
            onComplete(
              ...
              // Handling Future result on omitted here, done the same as before.
  ...
  implicit val mashaller = LogEntityMarshaller.create(maxJsObject)
